...
lexer_location := Source_Location{1, 0}
...
get_token :: proc() -> i32 {
    for {
        ...
        current_location = lexer_location
...
token_name :: proc(token: i32) -> string {
    switch token {
    case TOK_EOF:
        return "eof"
    case TOK_DEF:
    ...
    if token >= 0 && token < 128 {
        return fmt.tprintf("'%c'", rune(token))
    }
    return fmt.tprintf("token(%d)", token)
}
...
dump_expr :: proc(expr: ^Expr, indent := 0) {
    dump_indent(indent)
    if expr == nil {
        fmt.eprintln("<null>")
        return
    ...
    switch value in expr.node {
    case Number_Expr:
        fmt.eprintfln("number %.6f @%d:%d", value.value, expr.location.line, expr.location.column)
    case Variable_Expr:
        fmt.eprintfln("variable %s @%d:%d", value.name, expr.location.line, expr.location.column)
    case Unary_Expr:
        fmt.eprintfln("unary %s @%d:%d", token_name(value.opcode), expr.location.line, expr.location.column)
        dump_expr(value.operand, indent + 2)
    case Binary_Expr:
        fmt.eprintfln("binary %s @%d:%d", token_name(value.op), expr.location.line, expr.location.column)
        dump_expr(value.lhs, indent + 2)
        dump_expr(value.rhs, indent + 2)
    case Call_Expr:
        fmt.eprintfln("call %s @%d:%d", value.callee, expr.location.line, expr.location.column)
        for arg in value.args {
            dump_expr(arg, indent + 2)
        }
    case If_Expr:
        fmt.eprintfln("if @%d:%d", expr.location.line, expr.location.column)
        dump_expr(value.condition, indent + 2)
        dump_expr(value.then_expr, indent + 2)
        dump_expr(value.else_expr, indent + 2)
    case For_Expr:
        fmt.eprintfln("for %s @%d:%d", value.var_name, expr.location.line, expr.location.column)
        dump_expr(value.start, indent + 2)
        dump_expr(value.end, indent + 2)
        dump_expr(value.step, indent + 2)
        dump_expr(value.body, indent + 2)
    case Var_Expr:
        fmt.eprintfln("var @%d:%d", expr.location.line, expr.location.column)
        for binding in value.bindings {
            dump_indent(indent + 2)
            fmt.eprintfln("binding %s", binding.name)
            dump_expr(binding.init, indent + 4)
        }
        dump_expr(value.body, indent + 2)
    }
}

dump_prototype :: proc(proto: Prototype_AST, indent := 0) {
    dump_indent(indent)
    fmt.eprintfln("prototype %s @%d", proto.name, proto.line)
    for arg in proto.args {
        dump_indent(indent + 2)
        fmt.eprintfln("parameter %s", arg)
    }
}

dump_function :: proc(function_ast: Function_AST) {
    fmt.eprintln("function")
    dump_prototype(function_ast.proto, 2)
    dump_expr(function_ast.body, 2)
}
...
parse_if_expr :: proc() -> (^Expr, string) {
    ...
    result^ = {
        location = location,
        node     = If_Expr{condition, then_expr, else_expr},
    }
    return result, ""
}
...
parse_for_expr :: proc() -> (^Expr, string) {
    ...
    result^ = {
        location = location,
        node     = For_Expr{var_name, start, end, step, body},
    }
    return result, ""
}
...
parse_unary :: proc() -> (^Expr, string) {
    ...
    result^ = {
        location = location,
        node     = Unary_Expr{opcode, operand},
    }
    return result, ""
}
...
parse_bin_op_rhs :: proc(expression_precedence: int, lhs: ^Expr) -> (^Expr, string) {
    ...
    for {
        ...
        result^ = {
            location = location,
            node     = Binary_Expr{binary_op, left, rhs},
        }
        left = result
    }
}
...
parse_prototype :: proc() -> (Prototype_AST, string) {
    ...
    function_location := current_location
    kind := 0
    ...
    return Prototype_AST{function_name, arg_names, kind != 0, binary_precedence, function_location.line}, ""
}
...
parse_top_level_expr :: proc() -> (Function_AST, string) {
    ...
    return Function_AST{Prototype_AST{name = "main", line = location.line}, body}, ""
}
...
debug_builder: llvm.DIBuilderRef
compile_unit: llvm.MetadataRef
debug_file: llvm.MetadataRef
double_debug_type: llvm.MetadataRef
debug_scopes: [dynamic]llvm.MetadataRef
...
get_double_debug_type :: proc() -> llvm.MetadataRef {
    if double_debug_type == nil {
        double_debug_type = llvm.DIBuilderCreateBasicType(
            debug_builder,
            "double",
            len("double"),
            64,
            llvm.DWARFTypeEncoding(0x04),
            llvm.DIFlagZero,
        )
    }
    return double_debug_type
}

create_function_debug_type :: proc(arity: int) -> llvm.MetadataRef {
    types := make([]llvm.MetadataRef, arity + 1, context.temp_allocator)
    for &debug_type in types {
        debug_type = get_double_debug_type()
    }
    return llvm.DIBuilderCreateSubroutineType(
        debug_builder,
        debug_file,
        raw_data(types),
        u32(len(types)),
...
current_debug_scope :: proc() -> llvm.MetadataRef {
    if len(debug_scopes) == 0 {
        return compile_unit
    }
    return debug_scopes[len(debug_scopes) - 1]
}

emit_location :: proc(expr: ^Expr) {
    if expr == nil {
        llvm.SetCurrentDebugLocation2(llvm_builder, nil)
        return
    }

    location := llvm.DIBuilderCreateDebugLocation(
        llvm_context,
        u32(expr.location.line),
        u32(expr.location.column),
        current_debug_scope(),
        nil,
    )
    llvm.SetCurrentDebugLocation2(llvm_builder, location)
}

codegen_expr :: proc(expr: ^Expr) -> (llvm.ValueRef, string) {
    previous_location := llvm.GetCurrentDebugLocation2(llvm_builder)
    emit_location(expr)
    value, err := codegen_expr_impl(expr)
    llvm.SetCurrentDebugLocation2(llvm_builder, previous_location)
    return value, err
}

codegen_expr_impl :: proc(expr: ^Expr) -> (llvm.ValueRef, string) {
    switch value in expr.node {
    case Number_Expr:
    ...
    case Binary_Expr:
        if value.op == '=' {
            ...
            #partial switch lhs in value.lhs.node {
            case Variable_Expr:
...
codegen_function :: proc(function_ast: Function_AST) -> (llvm.ValueRef, string) {
    ...
    function_type := create_function_debug_type(len(function_ast.proto.args))
    if function_type == nil {
        llvm.DeleteFunction(function_value)
        return nil, "Could not create function debug type"
    }
    subprogram := llvm.DIBuilderCreateFunction(
        debug_builder,
        debug_file,
        fmt.ctprint(function_ast.proto.name),
        uint(len(function_ast.proto.name)),
        "",
        0,
        debug_file,
        u32(function_ast.proto.line),
        function_type,
        llvm.Bool(0),
        llvm.Bool(1),
        u32(function_ast.proto.line),
    ...
    if subprogram == nil {
        llvm.DeleteFunction(function_value)
        return nil, "Could not create function debug record"
    }
    llvm.SetSubprogram(function_value, subprogram)
    append(&debug_scopes, subprogram)
    defer pop(&debug_scopes)

    previous_location := llvm.GetCurrentDebugLocation2(llvm_builder)
    defer llvm.SetCurrentDebugLocation2(llvm_builder, previous_location)
    emit_location(nil)
    ...
    for argument_name, index in function_ast.proto.args {
        ...
        descriptor := llvm.DIBuilderCreateParameterVariable(
            debug_builder,
            subprogram,
            fmt.ctprint(argument_name),
            uint(len(argument_name)),
            u32(index + 1),
            debug_file,
            u32(function_ast.proto.line),
            get_double_debug_type(),
            llvm.Bool(1),
            llvm.DIFlagZero,
        )
        expression := llvm.DIBuilderCreateExpression(debug_builder, nil, 0)
        location := llvm.DIBuilderCreateDebugLocation(llvm_context, u32(function_ast.proto.line), 0, subprogram, nil)
        if descriptor == nil ||
           expression == nil ||
           location == nil ||
           llvm.DIBuilderInsertDeclareRecordAtEnd(debug_builder, variable, descriptor, expression, location, entry) ==
               nil {
            llvm.DeleteFunction(function_value)
            return nil, "Could not create parameter debug declaration"
        }
        llvm.BuildStore(llvm_builder, argument, variable)
        named_values[argument_name] = variable
    }

    emit_location(function_ast.body)
...
add_module_flag :: proc(name: string, value: u32) {
    constant := llvm.ConstInt(llvm.Int32TypeInContext(llvm_context), u64(value), llvm.Bool(0))
    llvm.AddModuleFlag(llvm_module, .Warning, fmt.ctprint(name), uint(len(name)), llvm.ValueAsMetadata(constant))
}

initialize_debug_info :: proc() -> bool {
    add_module_flag("Debug Info Version", llvm.DebugMetadataVersion())
    when ODIN_OS == .Darwin {
        add_module_flag("Dwarf Version", 2)
    }

    if !bool(llvm.IsNewDbgInfoFormat(llvm_module)) {
        process_failed = true
        fmt.eprintln("LLVM module does not support debug records")
        return false
    }

    debug_builder = llvm.CreateDIBuilder(llvm_module)
    if debug_builder == nil {
        process_failed = true
        fmt.eprintln("Could not create debug info builder")
        return false
    }
    debug_file = llvm.DIBuilderCreateFile(debug_builder, "fib.ks", len("fib.ks"), ".", len("."))
    if debug_file == nil {
        process_failed = true
        fmt.eprintln("Could not create debug source file")
        return false
    }
    producer := "Kaleidoscope Compiler"
    compile_unit = llvm.DIBuilderCreateCompileUnit(
        debug_builder,
        .C,
        debug_file,
        fmt.ctprint(producer),
        uint(len(producer)),
        llvm.Bool(0),
        "",
        0,
    ...
    if compile_unit == nil {
        process_failed = true
        fmt.eprintln("Could not create debug compile unit")
        return false
    }
    return true
}
...
print_module :: proc() {
    ...
    if text == nil {
        process_failed = true
        fmt.eprintln("Could not print LLVM module")
        return
    }
    defer llvm.DisposeMessage(text)
    fmt.eprint(string(text))
...
handle_extern :: proc() {
    ...
    _, codegen_err := codegen_prototype(proto)
    if codegen_err != "" {
        report_error(codegen_err)
        return
    }
...
run :: proc() {
    ...
    debug_scopes = make([dynamic]llvm.MetadataRef)
    defer {
        if debug_builder != nil {
            llvm.DisposeDIBuilder(debug_builder)
        }
    ...
    if !initialize_native_target_machine() || !initialize_debug_info() {
        return
    }
    main_loop()
    ...
    llvm.DIBuilderFinalize(debug_builder)
    if !verify_module() {
        return
    }
    print_module()
