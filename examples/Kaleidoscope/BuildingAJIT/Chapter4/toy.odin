...
clone_expr :: proc(source: ^Expr, allocator: mem.Allocator) -> ^Expr {
    if source == nil {
        return nil
    }
    result := new(Expr, allocator)
    ...
    case .Variable:
        result.name = strings.clone(source.name, allocator)
    case .Unary:
        result.operand = clone_expr(source.operand, allocator)
    case .Binary:
        result.lhs = clone_expr(source.lhs, allocator)
        result.rhs = clone_expr(source.rhs, allocator)
    case .Call:
        result.name = strings.clone(source.name, allocator)
        result.args = make([dynamic]^Expr, 0, len(source.args), allocator)
        for arg in source.args {
            append(&result.args, clone_expr(arg, allocator))
        }
    case .If:
        result.condition = clone_expr(source.condition, allocator)
        result.then_expr = clone_expr(source.then_expr, allocator)
        result.else_expr = clone_expr(source.else_expr, allocator)
    case .For:
        result.name = strings.clone(source.name, allocator)
        result.start = clone_expr(source.start, allocator)
        result.end = clone_expr(source.end, allocator)
        result.step = clone_expr(source.step, allocator)
        result.body = clone_expr(source.body, allocator)
    case .Var:
        result.bindings = make([dynamic]Var_Binding, 0, len(source.bindings), allocator)
        for binding in source.bindings {
            append(
                &result.bindings,
                Var_Binding{name = strings.clone(binding.name, allocator), init = clone_expr(binding.init, allocator)},
            )
        }
        result.body = clone_expr(source.body, allocator)
    case .Number:
...
destroy_expr :: proc(expr: ^Expr, allocator: mem.Allocator) {
    if expr == nil {
        return
    }
    switch expr.kind {
    case .Variable:
        delete(expr.name, allocator)
    case .Unary:
        destroy_expr(expr.operand, allocator)
    case .Binary:
        destroy_expr(expr.lhs, allocator)
        destroy_expr(expr.rhs, allocator)
    case .Call:
        delete(expr.name, allocator)
        for arg in expr.args {
        ...
        delete(expr.args)
    case .If:
        destroy_expr(expr.condition, allocator)
        destroy_expr(expr.then_expr, allocator)
        destroy_expr(expr.else_expr, allocator)
    case .For:
        delete(expr.name, allocator)
        destroy_expr(expr.start, allocator)
        destroy_expr(expr.end, allocator)
        destroy_expr(expr.step, allocator)
        destroy_expr(expr.body, allocator)
    case .Var:
        for binding in expr.bindings {
            delete(binding.name, allocator)
            destroy_expr(binding.init, allocator)
        }
        delete(expr.bindings)
        destroy_expr(expr.body, allocator)
    case .Number:
    }
    free(expr, allocator)
...
clone_function_ast :: proc(source: ^Function_AST, allocator: mem.Allocator) -> ^Function_AST {
    result := new(Function_AST, allocator)
    result.proto = clone_prototype(source.proto, allocator)
    result.body = clone_expr(source.body, allocator)
    return result
}

destroy_function_ast :: proc(fn: ^Function_AST, allocator: mem.Allocator) {
    if fn == nil {
        return
    }
    destroy_prototype(&fn.proto, allocator)
    destroy_expr(fn.body, allocator)
    free(fn, allocator)
}

irgen_and_take_ownership :: proc(fn_ast: ^Function_AST) -> (tsm: llvm.OrcThreadSafeModuleRef, err: Frontend_Error) {
    if _, codegen_err := codegen_function(fn_ast); codegen_err != nil {
        return nil, codegen_err
    }
...
handle_definition :: proc() {
    ...
    err := jit_add_ast(the_jit, fn_ast)
