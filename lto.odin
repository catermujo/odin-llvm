...
foreign import lto_lib {LTO_SYSTEM_LIB when LINK == "system" else "windows_x64/LTO.lib" when ODIN_OS == .Windows else "darwin_x64/libLTO.dylib" when ODIN_OS == .Darwin && ODIN_ARCH == .amd64 else "darwin_arm64/libLTO.dylib" when ODIN_OS == .Darwin && ODIN_ARCH == .arm64 else "linux_x64/libLTO.so" when ODIN_OS == .Linux && ODIN_ARCH == .amd64 else "linux_arm64/libLTO.so"}
import "core:sys/posix"

_ :: posix

lto_bool_t :: u8 when ODIN_OS == .Windows else bool

when ODIN_OS == .Windows {
    lto_off_t :: distinct i64
    posix_off_t :: lto_off_t
} else {
    lto_off_t :: posix.off_t
}
...
*/
lto_symbol_attributes :: enum LLVM_C_Enum {
...
* \since prior to LTO_API_VERSION=3
*/
lto_debug_model :: enum LLVM_C_Enum {
    NONE  = 0,
    DWARF = 1,
}
...
* \since prior to LTO_API_VERSION=3
*/
lto_codegen_model :: enum LLVM_C_Enum {
    STATIC         = 0,
    DYNAMIC        = 1,
    DYNAMIC_NO_PIC = 2,
    DEFAULT        = 3,
...
foreign lto_lib {
	...
	* \since LTO_API_VERSION=5
	*/
    lto_module_create_from_fd_at_offset :: proc(fd: i32, path: cstring, file_size: c.size_t, map_size: c.size_t, offset: lto_off_t) -> lto_module_t ---
...
*/
lto_codegen_diagnostic_severity_t :: enum LLVM_C_Enum {
    ERROR   = 0,
    WARNING = 1,
